...
- Throughout the repository, the name `testTask_env` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of testTask_env (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/testTask_env
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/testTask_env/testTask_env/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/testTask_env"
    ]
}
```
